...
bool MsckfVio::createRosIO() {
  ...
  leica_sub = nh.subscribe("/leica/position",15,&MsckfVio::leicaCallback, this);

  odom_pub = nh.advertise<nav_msgs::Odometry>("odom", 10);
  feature_pub = nh.advertise<sensor_msgs::PointCloud2>(
      "feature_point_cloud", 10);

  true_pose_pub = nh.advertise<geometry_msgs::PointStamped>("leica_positon", 15);
  ...
  const geometry_msgs::PointStamped& msg) {
    ...
    if ( is_first_leica ){
      if (msg.point.x!=0 & msg.point.y!=0 & msg.point.z != 0){
        leica_offset_x = msg.point.x;
        leica_offset_y = msg.point.y;
        leica_offset_z = msg.point.z;
        is_first_leica = false;
      }
    }else{
      leica_msg_buffer = msg;
      leica_msg_buffer.point.x -= leica_offset_x;
...
void MsckfVio::publish(const ros::Time& time) {
  ...
  true_pose_pub.publish(leica_msg_buffer);
